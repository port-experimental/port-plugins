...
7. README preview uses GitHub `user-attachments` URL (`width`, `height`, descriptive `alt`) — no local screenshot paths.
...
| Preview image | GitHub `user-attachments` URL with `width`/`height`/`alt` — no local `assets/preview.png` — [readme-and-audit.md](references/readme-and-audit.md) (**Preview image**) |

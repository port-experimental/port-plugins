...
6. **UX and UI are first-class** — loading, empty, and error states; theme via `applyThemeCss()`; responsive full-iframe layout; accessible controls; **decoration colors on their class**, not `:root` `--accent` (see [Surface vs decoration colors](scaffolding-and-implementation.md#surface-vs-decoration-colors)).

 ...
 *
 * Add a new framework by dropping a definition file next to `space.ts` and
 * registering it here - that is the only code change needed to ship another
 * survey type. At runtime a `survey` entity's own `definition` property always
 * takes precedence over these (see `resolveDefinition`), so most surveys never
...
/** Built-in fallback used when a survey entity carries no inline definition. */
export const DEFAULT_TEMPLATE = SPACE_SURVEY;

/** Look up a template by definition id (e.g. "space") or framework (e.g. "SPACE"). */
...
/**
 * Resolve the definition that should drive the form, in priority order:
 *   1. inline `definition` stored on the survey entity (full abstraction);
 *   2. a built-in template matched by the entity's `framework`;
...
/** Validate the loosely-typed `definition` object from Port into a usable shape. */

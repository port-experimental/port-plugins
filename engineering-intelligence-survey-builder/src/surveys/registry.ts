...
import { DORA_SURVEY } from "./dora";
import { DX_CORE_4_SURVEY } from "./dx-core-4";

/**
 * Built-in survey templates an author can clone as a starting point.
 *
 * Each is a full `SurveyDefinition` - the exact shape that ends up on a saved
 * survey entity's `definition` property, which the runner (developer-survey)
 * and analytics plugins already consume. Add another framework by dropping a
 * definition file next to these and registering it here.
 */
export type Template = {
  /** Stable key + the survey.framework enum value it maps to. */
  framework: string;
  label: string;
  blurb: string;
...
export const TEMPLATES: Template[] = [
  {
    framework: "SPACE",
    label: "SPACE",
    blurb: "Satisfaction, Performance, Activity, Communication, Efficiency - the classic DevEx pulse.",
    definition: SPACE_SURVEY,
  },
  {
    framework: "AI Adoption",
    label: "AI Adoption & Impact",
    blurb: "Outcome-focused read on AI tooling: adoption, velocity, time saved, quality, impact.",
    definition: AI_ADOPTION_SURVEY,
  },
  {
    framework: "DORA",
    label: "DORA",
    blurb: "Self-reported deployment frequency, lead time, change failure rate, and time to restore.",
    definition: DORA_SURVEY,
  },
  {
    framework: "DX Core 4",
    label: "DX Core 4",
    blurb: "Speed, Effectiveness, Quality, Impact - the unified DORA + SPACE + DevEx framework.",
    definition: DX_CORE_4_SURVEY,
  },
];
...
/** Deep clone so editing a draft never mutates the shipped template object. */
export function cloneDefinition(def: SurveyDefinition): SurveyDefinition {
  return JSON.parse(JSON.stringify(def)) as SurveyDefinition;
}

/** An empty survey to build up from scratch (the "blank" starting point). */
export function blankDefinition(): SurveyDefinition {
  return {
    id: "custom",
...
/**
 * Validate the loosely-typed `definition` object read back from a survey entity
 * into a usable shape (mirrors the runner's normalizer so round-trips are safe).
 */
export function normalizeDefinition(raw: unknown): SurveyDefinition | null {
  ...
  // No flat top-level questions? Recover a nested (e.g. Port AI) definition so
  // it can be edited here and re-saved in the canonical flat shape.
...
function slugifyChoice(s: string): string {
  return s
    .toLowerCase()
    .trim()
    .replace(/[^a-z0-9]+/g, "-")
...
function coerceChoices(raw: unknown): Choice[] | undefined {
  ...
  const choices = raw
    .map((o): Choice | null => {
      if (typeof o === "string") return { value: slugifyChoice(o) || o, label: o };
      if (o && typeof o === "object") {
        ...
        const value =
          typeof obj.value === "string" ? obj.value : slugifyChoice(label) || label;

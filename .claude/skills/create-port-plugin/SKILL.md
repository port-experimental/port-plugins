...
Strategic defaults — tactical checklist in **Non-negotiables**; anti-patterns in [guidelines.md](.cursor/skills/create-port-plugin/references/guidelines.md).
...
2. **UX and UI matter** — loading, empty, and error states; theme alignment; full-iframe layout. See [scaffolding-and-implementation.md](.cursor/skills/create-port-plugin/references/scaffolding-and-implementation.md) (**UX and UI**).
...
7. **Charts → Recharts** — when the widget needs bar, line, area, pie, donut, or similar data visualization, prefer **[Recharts](https://recharts.org/)** over hand-rolled SVG/CSS charts. Apply [webpack-port-upload-safety.md](.cursor/skills/create-port-plugin/references/webpack-port-upload-safety.md) when adding Recharts. Details: [scaffolding-and-implementation.md](.cursor/skills/create-port-plugin/references/scaffolding-and-implementation.md) (**Charts and data visualization**).

## Overview

Plugins in this repo are **custom widgets**: each compiles to a single `dist/index.html` (webpack, inlined assets), runs in a Port `<iframe>`, and is registered via the CLI. Use this skill to **reuse** an existing plugin (configure or recommend as-is) or **create** a new one from the bundled templates. Params live in `upload-params.json`; see `.cursor/skills/create-port-plugin/assets/` and `.cursor/skills/create-port-plugin/references/`.

## Choose a workflow

| Goal | Follow |
|------|--------|
| **Reuse** an existing plugin (exact or superset match — configure, don’t rebuild) | [references/reuse-workflow.md](.cursor/skills/create-port-plugin/references/reuse-workflow.md) (stop when a match is found) |
| **Audit or align** an existing plugin (README, params, SDK, build, upload docs) | [references/readme-and-audit.md](.cursor/skills/create-port-plugin/references/readme-and-audit.md) |
| **Copy, adapt, or scaffold** a new plugin | Steps below → [references/reuse-workflow.md](.cursor/skills/create-port-plugin/references/reuse-workflow.md) then [references/scaffolding-and-implementation.md](.cursor/skills/create-port-plugin/references/scaffolding-and-implementation.md) |
...
Else → Scaffold from .cursor/skills/create-port-plugin/assets/ (scaffolding-and-implementation.md)
        ↓
...
Full decision matrix, checklist, and examples: **[references/reuse-workflow.md](.cursor/skills/create-port-plugin/references/reuse-workflow.md)**.
...
**Relation resolution order (runtime):** `PLUGIN_DATA.entity` → `POST .../entities/search` (`relatedTo` / relation rules) → pick unambiguously from `GET /v1/blueprints/{identifier}` using the **designed** relation identifiers → optional string override **only** when the user explicitly needs multi-tenant relation-key variance the catalog cannot normalize. Details: [scaffolding-and-implementation.md](.cursor/skills/create-port-plugin/references/scaffolding-and-implementation.md) (**Catalog over plugin params**, **Prefer discovering relations**).

### 3. Scaffold or adapt implementation

- **Templates:** copy verbatim/adapt from [assets/](.cursor/skills/create-port-plugin/assets/) per [references/scaffolding-and-implementation.md](.cursor/skills/create-port-plugin/references/scaffolding-and-implementation.md).
- **Host bridge, local dev mocks, API calls, search body shape, portal URLs, params, relations, layout, theming:** same reference + [references/plugin-architecture.md](.cursor/skills/create-port-plugin/references/plugin-architecture.md).
- **Repo layout, naming, optional CI upload:** [references/widget-conventions.md](.cursor/skills/create-port-plugin/references/widget-conventions.md).
...
- Webpack `inject: "body"` and root `height: 100%` — [references/plugin-architecture.md](.cursor/skills/create-port-plugin/references/plugin-architecture.md) (Critical Webpack/CSS)
- Entity search: `{ query: { combinator, rules } }` on `POST /v1/blueprints/{blueprint}/entities/search`
- `applyThemeCss()` when using the SDK

### 5. Document and ship

- **Bump `package.json` `version` once per branch** when the plugin has functional changes — based on **all** changes on the branch since merge base, **not** once per agent invocation or per feature while building. **New plugins:** stay at **`0.1.0`** for the whole first branch (do not stack `0.2.0`, `0.3.0`, … during development). Workflow: [readme-and-audit.md](.cursor/skills/create-port-plugin/references/readme-and-audit.md) (**Versioning — once per branch**, **Greenfield plugin**).
- Per-plugin **README** (required section order): [references/readme-and-audit.md](.cursor/skills/create-port-plugin/references/readme-and-audit.md)
- Repo-level widgets table row when adding a new plugin (include **Version** from that plugin’s `package.json` `version` field)
- Canonical **`port-plugins upload`** command — exact flags in [references/readme-and-audit.md](.cursor/skills/create-port-plugin/references/readme-and-audit.md) (Setup → Upload); do not invent CLI flags

### 6. Review anti-patterns and persistence

Before finishing: [references/guidelines.md](.cursor/skills/create-port-plugin/references/guidelines.md) (anti-patterns, Port vs `localStorage`, troubleshooting table).
...
3. Confirm per-plugin README section order and upload command per [readme-and-audit.md](.cursor/skills/create-port-plugin/references/readme-and-audit.md).
4. Walk the **Non-negotiables** table and [guidelines.md](.cursor/skills/create-port-plugin/references/guidelines.md) anti-patterns.
5. If the branch has functional plugin changes, confirm **`package.json` `version`** was bumped **once for the branch** (not re-bumped every invocation) and the root **`README.md` Plugins** **Version** cell matches — [readme-and-audit.md](.cursor/skills/create-port-plugin/references/readme-and-audit.md) (**Versioning — once per branch**).
...
| Charts | **Recharts** for bar/line/area/pie/donut and similar viz — not custom SVG/CSS chart implementations unless trivial (e.g. a single progress bar). Webpack upload safety when Recharts is added — [scaffolding-and-implementation.md](.cursor/skills/create-port-plugin/references/scaffolding-and-implementation.md) (**Charts**), [webpack-port-upload-safety.md](.cursor/skills/create-port-plugin/references/webpack-port-upload-safety.md) |
...
| README prerequisites | Tables for catalog **and** other Port setup (relations, automations, SSA, integrations, …) — see [readme-and-audit.md](.cursor/skills/create-port-plugin/references/readme-and-audit.md) |
| Search | Nested `query`; not top-level `combinator`/`rules` |
| Dashboard page filters | On `POST .../entities/search` with a `query`, merge `page.pageFilters` via **`mergePageFilters`**; pass the **full blueprint** from **`params.blueprint.value`** as the third argument (not `{ identifier }` only) — [plugin-architecture.md](.cursor/skills/create-port-plugin/references/plugin-architecture.md) |
...
| Upload CLI | `port-plugins upload` with **`--file`**, **`--identifier`**, **`--title`**, **`--params`**, **`--description`**, **`--upsert`** only — see [readme-and-audit.md](.cursor/skills/create-port-plugin/references/readme-and-audit.md) |
| Versioning | **Once per git branch** per plugin: bump from merge-base semver using **cumulative** changes — **not** per agent run or per feature. **Greenfield:** **`0.1.0`** for the entire first branch until publish/merge; see [readme-and-audit.md](.cursor/skills/create-port-plugin/references/readme-and-audit.md) (**Versioning — once per branch**, **Greenfield plugin**) |

## Reference index

| File | Contents |
|------|----------|
| [reuse-workflow.md](.cursor/skills/create-port-plugin/references/reuse-workflow.md) | Reuse steps 1–5, blueprint matrix, checklist, adapt vs create, code-reuse patterns |
| [scaffolding-and-implementation.md](.cursor/skills/create-port-plugin/references/scaffolding-and-implementation.md) | Templates, `App.tsx` implementation, params, relations, code layout, **charts (Recharts)**, **optional palette + `-bg`/`-text` variants** |
| [readme-and-audit.md](.cursor/skills/create-port-plugin/references/readme-and-audit.md) | Auditing existing plugins, PR checklist, per-plugin README §8 |
| [plugin-architecture.md](.cursor/skills/create-port-plugin/references/plugin-architecture.md) | postMessage, `PLUGIN_DATA`, API, theming, build/deploy, local dev |
| [webpack-port-upload-safety.md](.cursor/skills/create-port-plugin/references/webpack-port-upload-safety.md) | `globalObject` / lodash shim — **required with Recharts**; also when upload rejects `Function` |
| [widget-conventions.md](.cursor/skills/create-port-plugin/references/widget-conventions.md) | Directory layout, naming, optional automation |
| [guidelines.md](.cursor/skills/create-port-plugin/references/guidelines.md) | Anti-patterns, data persistence, troubleshooting |
...
| `webpack/lodash-root-shim.js` | `<widget>/webpack/lodash-root-shim.js` — **required** when using Recharts; see [webpack-port-upload-safety.md](.cursor/skills/create-port-plugin/references/webpack-port-upload-safety.md) |
